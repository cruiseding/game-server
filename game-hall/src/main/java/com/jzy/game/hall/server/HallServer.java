...
public class HallServer implements Runnable {
	...
	public HallServer(String configPath) {
		// 加载连接大厅客户端配置
		ThreadPoolExecutorConfig hallClientThreatPool = FileUtils.getConfigXML(configPath,
				"hallClientThreadPoolExecutorConfig.xml", ThreadPoolExecutorConfig.class);
		if (hallClientThreatPool == null) {
			LOGGER.error("{}/hallClientThreadPoolExecutorConfig.xml未找到", configPath);
			System.exit(0);
		}
		MinaClientConfig minaClientConfig_gate = FileUtils.getConfigXML(configPath, "minaClientConfig_gate.xml",
